...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        // Petla na zliczanie pozycji tabeli
        for(int i=0; i<3; i++) {
            //warunek na kolumny czy są 3 znaki w lini + result
            if ( grid[i][0] !='-' && grid[i][0] == grid[i][1] && grid[i][1] == grid[i][2]){
                result = grid[i][0] + " wins";
                //warunek na wiersze czy są 3 znaki w lini + result
            } else if (grid[0][i] !='-' && grid[0][i] == grid[1][i] && grid[1][i] == grid[2][i]){
                result = grid[0][i] + " wins";
                //warunek na skosy czy są 3 znaki w lini + result
            }else if ((grid[0][0] !='-' && grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2])
                    ||(grid[0][2] !='-' && grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0])) {
                result = grid[1][1] + " wins";
            }
            //warunek na weryfikacje czy cała tablica znaków ma wartości nadane(nie są puste)
            // weryfikacja czy nikt nie wygrał w ostatnim ruchu(result byłby win)
            if (grid[0][0] != '-' && grid[0][1]!= '-' && grid[0][2]!= '-' && grid[1][0]!= '-'
                    && grid[1][1]!= '-' && grid[1][2]!= '-' && grid[2][0]!= '-' && grid[2][1]!= '-'
                    && grid[2][2] != '-' && result == "None") {
                result = "Tie";
        }
        }
         return result;
